...
const essentialsValueConverter = {
    ...
    faultCode: {
        from: (value: string, meta: Fz.Meta) => {
            logger.warn(`zigbee-herdsman-converters:essentialsThermostat: ERROR CODE received: ${JSON.stringify(value)}`);
            return value;
        },
    },
    away_setting: {
        ...
        to: (value: {
            ...
            logger.debug(JSON.stringify({ 'send to tuya': output, 'value was': value }));
        },
    },
    day_schedule: (day: string, index: number) => {
        return {
            ...
            to: (value: KeyValue, meta: Tz.Meta) => {
                ...
                logger.debug(`schedule update received for ${day}: ${JSON.stringify(value)}`);
                logger.debug(`old schedule for ${day}: ${JSON.stringify(meta.state[day])}`);
                ...
                for (let i = 0; i < 9; i++) {
                    ...
                    logger.debug(`setpoint_${i + 1} temperature:${temperature} hour:${hour} minute:${minute})`);
